...
TracerBaseRos::TracerBaseRos(std::string node_name)
  ...
  this->declare_parameter<bool>("is_tracer_mini", false);
...
void TracerBaseRos::LoadParameters() {
  ...
  std::cout << "- is tracer mini: " << std::boolalpha << is_tracer_mini_ << std::endl;
  std::cout << "- simulated robot: " << std::boolalpha << simulated_robot_ << std::endl;
...
bool TracerBaseRos::Initialize() {
  ...
  if (detector.Connect(port_name_)) {
    ...
    if (proto == ProtocolVersion::AGX_V2) {
      ...
      std::cout << "Creating interface for Tracer with AGX_V2 Protocol" << std::endl;  
    } else {
