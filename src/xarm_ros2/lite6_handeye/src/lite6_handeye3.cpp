...
bool XArmHandEyeCalibration::moveXArmToPose(const std::array<double, 6>& pose) {
    ...
    if (future.wait_for(std::chrono::seconds(10)) == std::future_status::ready) {
        ...
        RCLCPP_ERROR(this->get_logger(), "Move command timeout!");
        return false;
    }
...
void XArmHandEyeCalibration::performHandEyeCalibration() {
    ...
    for (const auto& pose : trajectories) {
        ...
        rclcpp::sleep_for(std::chrono::seconds(1)); // 給時間讓訂閱更新
...
int main(int argc, char** argv) {
    ...
    // 使用 MultiThreadedExecutor 讓所有 callback 可並行運行
    rclcpp::executors::MultiThreadedExecutor executor;
    executor.add_node(node);

    // 讓 Executor 自動處理所有回調，確保影像訂閱 & 服務請求正常運行
    executor.spin();  
